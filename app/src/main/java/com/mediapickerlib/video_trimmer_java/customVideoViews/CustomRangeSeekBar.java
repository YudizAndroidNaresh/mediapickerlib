...
public class CustomRangeSeekBar extends View {
    ...
    private void init() {
      ...
      //  mThumbHeight = BarThumb.getHeightBitmap(mBarThumbs);
        ...
        int lineColor = ContextCompat.getColor(getContext(), R.color.white);
        mLine.setAntiAlias(true);
        mLine.setStrokeCap(Paint.Cap.SQUARE);
        mLine.setStrokeWidth(4.0F);
        mLine.setColor(lineColor);
        mLine.setStyle(Paint.Style.STROKE);

        int redColor = ContextCompat.getColor(getContext(), R.color.red);
        mRedLine.setAntiAlias(true);
        mRedLine.setColor(redColor);
    ...
    public void initMaxWidth() {
       ...
       // (mStartPosition+30) * 100) / mDuration
       // mMaxWidth = ((30*100)/duration)+1000;
    //mBarThumbs.get(1).getPos() - mBarThumbs.get(0).getPos();
...
//                        checkPositionThumb(mBarThumb2, mBarThumb, dx, false, coordinate);
    ...
    private void drawShadow(@NonNull Canvas canvas) {
        if (!mBarThumbs.isEmpty()) {
            ...
            final float start = mBarThumbs.get(0).getPos();
            final float end = mBarThumbs.get(1).getPos();
            Rect linesRect=new Rect((int) (start+ (mThumbWidth / 2)+20),(int) (((mThumbHeight - mHeightTimeLine) / 2) + 3),(int) end,mHeightTimeLine + (int) (mThumbHeight - mHeightTimeLine) / 2 - 3);
            canvas.drawRect(linesRect, mLine);
            //int startSeek=(int) (start+ (mThumbWidth / 2) +20);
         //   Rect redlinesRect=new Rect(startSeek+startPostionRedLine++,(int) (((mThumbHeight - mHeightTimeLine) / 2) + 3),startSeek+6,mHeightTimeLine + (int) (mThumbHeight - mHeightTimeLine) / 2 - 3);
         // canvas.drawRect(redlinesRect, mRedLine);
        }
    }
    private void reset(){
